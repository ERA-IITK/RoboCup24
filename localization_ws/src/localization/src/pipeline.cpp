...
public:
    ...
    {
        image_subscriber_ = create_subscription<sensor_msgs::msg::Image>("/camera/image_raw",10,std::bind(&LineSubscriber::imageCallback, this, std::placeholders::_1));
        subscription_ = this->create_subscription<sensor_msgs::msg::Image>("/camera/depth/image_raw", 10, std::bind(&LineSubscriber::depth_callback, this, std::placeholders::_1));
        camera_info_subscriber_ = create_subscription<sensor_msgs::msg::CameraInfo>("/camera/camera_info",10,std::bind(&LineSubscriber::cameraInfoCallback, this, std::placeholders::_1));
    }
    ...
    void depth_callback(const sensor_msgs::msg::Image::SharedPtr msg)
    {
        ...
        for (const auto &contour : contours)
        {
            for (const auto &point : contour)
            {
                ...
                myfile << "(" << point.x << ", " << point.y << ") is (" << odom.x + worldCoords.z()*cos(thee) + worldCoords.x()*sin(thee) << "," << odom.y + worldCoords.z()*sin(thee) - worldCoords.x()*cos(thee) << ")\n";
...
void localization_thread()
{
    ...
    // two.x = 0.02, two.y = -0.03, two.theta = 3;
    // three.x = 0, three.y = 0.01, three.theta = -2;
    // four.x = -0.01, four.y = 0, four.theta = -1;
    ...
    int k = 0;
    std::string imagePath = "/home/legendarygene/Desktop/RoboCup24/localization_ws/src/localization/images/robocup_field.png";
